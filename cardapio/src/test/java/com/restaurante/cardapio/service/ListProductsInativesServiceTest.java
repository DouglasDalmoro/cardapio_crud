...
import static com.restaurante.cardapio.factory.ProductFactory.createProductInative;
import static org.junit.jupiter.api.Assertions.assertEquals;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;

@ExtendWith(MockitoExtension.class)
public class ListProductsInativesServiceTest {

    @InjectMocks
    private ListProductsInactiveService test;

    @Mock
    private ProductRepository productRepository;

    @Test
    @DisplayName("It should return an empty product list if it does not have inatives courses")
    void  shouldReturnEmptyProductList(){
        ...
        when(productRepository.findAllByActive(false, pageable))
                .thenReturn(productsResearched);

        Page<ProductResponse> response = test.list(pageable);

        verify(productRepository).findAllByActive(false, pageable);
        assertEquals(products.size(), response.getSize());
    }

    @Test
    @DisplayName("Should return a list of products if it has inactive courses")
    void  shouldReturnProductList(){

        List<Product> products = List.of(
                createProductInative(),
                createProductInative()
        );
        Page<Product> productsResearched = new PageImpl<>(products);
        PageRequest pageable = PageRequest.of(0, 10);
